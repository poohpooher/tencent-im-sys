 ...
 *
 *  直播群（AVChatRoom）的限制：
 *  - 设置 filter 为 V2TIM_GROUP_MEMBER_FILTER_ADMIN，SDK 会返回所有管理员；设置 filter 为 SDK 定义的其他值，SDK 返回全部成员。返回的人数规则为：拉取最近入群群成员最多 1000 人，新进来的成员排在前面，需要升级旗舰版，并且在 [控制台](https://console.cloud.tencent.com/im) 开启“直播群在线成员列表”开关（6.3 及以上版本支持）。
 ...
 *  - 群成员资料信息仅支持 userID | nickName | faceURL 字段。
 *
 * __示例__
 * @code{.cpp}
...
// bool, 只写(选填), 是否采取分页方式返回，默认值 false; 如果需要分页返回，请将该字段设置为 true
static const char* kTIMGroupPendencyOptionFetchInPages = "group_pendency_option_fetch_in_pages";
